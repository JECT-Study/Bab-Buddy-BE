...
public class RecommendFoodController {
    ...
    @Operation(summary = "예비 음식 조회", description = "예비 음식 2개를 조회합니다.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "음식점 3개 조회 성공"),
            @ApiResponse(responseCode = "204", description = "데이터 없음"),
            @ApiResponse(responseCode = "404", description = "음식 존재하지 않음"),
    })
    @GetMapping("/prepared/{foodId}")
    public ResponseEntity<List<RecommendPreparedFoodRes>> getPreparedFood(
            @PathVariable Long foodId) {
        return ResponseEntity.ok(recommendFoodService.preparedFood(foodId));
    }
